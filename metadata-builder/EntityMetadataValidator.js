    ...
    EntityMetadataValidator.prototype.validate = function (entityMetadata, allEntityMetadatas, driver) {
        ...
        if (entityMetadata.inheritancePattern === "STI" || entityMetadata.tableType === "entity-child") {
            if (!entityMetadata.discriminatorColumn)
                throw new error_1.TypeORMError("Entity ".concat(entityMetadata.name, " using single-table inheritance, it should also have a discriminator column. Did you forget to put discriminator column options?"));
            if (typeof entityMetadata.discriminatorValue === "undefined")
                throw new error_1.TypeORMError("Entity ".concat(entityMetadata.name, " has an undefined discriminator value. Discriminator value should be defined."));
            ...
            if (sameDiscriminatorValueEntityMetadata)
                throw new error_1.TypeORMError("Entities ".concat(entityMetadata.name, " and ").concat(sameDiscriminatorValueEntityMetadata.name, " have the same discriminator values. Make sure they are different while using the @ChildEntity decorator."));
        }
        ...
        if (!(driver instanceof MongoDriver_1.MongoDriver)) {
            entityMetadata.columns.forEach(function (column) {
                ...
                if (column.length && driver.withLengthColumnTypes.indexOf(normalizedColumn) === -1)
                    throw new error_1.TypeORMError("Column ".concat(column.propertyName, " of Entity ").concat(entityMetadata.name, " does not support length property."));
                if (column.type === "enum" && !column.enum && !column.enumName)
                    throw new error_1.TypeORMError("Column \"".concat(column.propertyName, "\" of Entity \"").concat(entityMetadata.name, "\" is defined as enum, but missing \"enum\" or \"enumName\" properties."));
            });
        }
        if (driver instanceof MysqlDriver_1.MysqlDriver || driver instanceof AuroraDataApiDriver_1.AuroraDataApiDriver) {
            ...
            if (generatedColumns.length > 1)
                throw new error_1.TypeORMError("Error in ".concat(entityMetadata.name, " entity. There can be only one auto-increment column in MySql table."));
        }
        ...
        entityMetadata.relations.forEach(function (relation) {
            ...
            if (isCircularCascadeRemove)
                throw new error_1.TypeORMError("Relation ".concat(entityMetadata.name, "#").concat(relation.propertyName, " and ").concat(relation.inverseRelation.entityMetadata.name, "#").concat(relation.inverseRelation.propertyName, " both has cascade remove set. ") +
