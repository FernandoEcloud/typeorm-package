...
var AbstractSqliteQueryRunner = /** @class */ (function (_super) {
            ...
            return (0, tslib_1.__generator)(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 3:
                        ...
                        up = new Query_1.Query("ALTER TABLE \"".concat(oldTable.name, "\" RENAME TO \"").concat(newTableName, "\""));
                        down = new Query_1.Query("ALTER TABLE \"".concat(newTableName, "\" RENAME TO \"").concat(oldTable.name, "\""));
            ...
            return (0, tslib_1.__generator)(this, function (_j) {
                switch (_j.label) {
                    ...
                    case 1:
                        ...
                        return [3 /*break*/, 4];
                    case 2:
                        tableNamesString_1 = tableNames.map(function (tableName) { return "'".concat(tableName, "'"); }).join(", ");
                        tablesSql = "SELECT * FROM \"sqlite_master\" WHERE \"type\" = 'table' AND \"name\" IN (".concat(tableNamesString_1, ")");
    ...
    AbstractSqliteQueryRunner.prototype.createTableSql = function (table, createForeignKeys) {
        ...
        if (table.foreignKeys.length > 0 && createForeignKeys) {
            var foreignKeysSql = table.foreignKeys.map(function (fk) {
                ...
                var constraint = "CONSTRAINT \"".concat(fk.name, "\" FOREIGN KEY (").concat(columnNames, ") REFERENCES \"").concat(fk.referencedTableName, "\" (").concat(referencedColumnNames, ")");
                if (fk.onDelete)
    ...
    AbstractSqliteQueryRunner.prototype.createIndexSql = function (table, index) {
        ...
        return new Query_1.Query("CREATE ".concat(index.isUnique ? "UNIQUE " : "", "INDEX \"").concat(index.name, "\" ON \"").concat(table.name, "\" (").concat(columns, ") ").concat(index.where ? "WHERE " + index.where : ""));
    };
    /**
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        if (migrateData) {
                            ...
                            upQueries.push(new Query_1.Query("INSERT INTO \"".concat(newTable.name, "\"(").concat(newColumnNames, ") SELECT ").concat(oldColumnNames, " FROM \"").concat(oldTable.name, "\"")));
                            downQueries.push(new Query_1.Query("INSERT INTO \"".concat(oldTable.name, "\"(").concat(oldColumnNames, ") SELECT ").concat(newColumnNames, " FROM \"").concat(newTable.name, "\"")));
                        }
                        // drop old table
                        upQueries.push(this.dropTableSql(oldTable));
                        downQueries.push(this.createTableSql(oldTable, true));
                        // rename old table
                        upQueries.push(new Query_1.Query("ALTER TABLE \"".concat(newTable.name, "\" RENAME TO \"").concat(oldTable.name, "\"")));
                        downQueries.push(new Query_1.Query("ALTER TABLE \"".concat(oldTable.name, "\" RENAME TO \"").concat(newTable.name, "\"")));
